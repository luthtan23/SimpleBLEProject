package com.luthtan.simplebleproject.dashboard_feature.ble
...
object ConnectionManager {
    ...
    private fun doNextOperation() {
        ...
        if (operation is Connect) {
            with(operation) {
                Log.e(TAG, "Connecting to ${device.address}")
    ...
    private val callback = object : BluetoothGattCallback() {
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (newState == BluetoothProfile.STATE_CONNECTED) {
                    Log.e(TAG, "onConnectionStateChange: connected to $deviceAddress")
        ...
        override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
            with(gatt) {
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    Log.e(TAG, "Discovered ${services.size} services for ${device.address}.")
                    printGattTable()
        ...
        override fun onMtuChanged(gatt: BluetoothGatt, mtu: Int, status: Int) {
            Log.e(TAG, "ATT MTU changed to $mtu, success: ${status == BluetoothGatt.GATT_SUCCESS}")
            ...
            with(characteristic) {
                when (status) {
                    BluetoothGatt.GATT_SUCCESS -> {
                        Log.d(TAG, "Read characteristic $uuid | value: ${value.toHexString()}")
        ...
        override fun onDescriptorRead(
            ...
            with(descriptor) {
                when (status) {
                    BluetoothGatt.GATT_SUCCESS -> {
                        Log.d(TAG, "Read descriptor $uuid | value: ${value.toHexString()}")
        ...
        override fun onDescriptorWrite(
            ...
            with(descriptor) {
                when (status) {
                    BluetoothGatt.GATT_SUCCESS -> {
                        Log.d(TAG, "Wrote to descriptor $uuid | value: ${value.toHexString()}")
        ...
        private fun onCccdWrite(
            ...
            when {
                notificationsEnabled -> {
                    Log.e(TAG, "Notifications or indications ENABLED on $charUuid")
                    listeners.forEach {
                ...
                notificationsDisabled -> {
                    Log.e(TAG, "Notifications or indications DISABLED on $charUuid")
                    listeners.forEach {
        ...
        override fun onReceive(context: Context, intent: Intent) {
            with(intent) {
                if (action == BluetoothDevice.ACTION_BOND_STATE_CHANGED) {
                    ...
                    Log.e(TAG, "${device?.address} bond state changed | $bondTransition")
                }
            }
        }
